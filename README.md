...
国が定めた仮名遣いの基準「現代仮名遣い」に基づき、四つ仮名（ぢ・づ）・オ列長音・
助詞「は／を／へ」・動詞「いう」の表記など、機械的に判定できる仮名遣いの誤りを、
小説エディタ [illusions](https://illusions.app/) の自動校正でチェックします。
...
| オ列長音の例外語 | 「お」を添えて書く例外語が「おう」と誤記された箇所を検出 | `おうかみ` → `おおかみ` |
| 助詞「は・を・へ」の表記（L2） | 助詞を「わ・お・え」と書いた箇所を形態素解析で検出 | `これお見て` → `これを見て` |
| 動詞「いう」の表記（L2） | 「言う／いう」を「ゆう」と書いた箇所を形態素解析で検出 | `そうゆう話` → `そういう話` |
...
- 本ルールセットは、このうち機械的に判定できる規定——四つ仮名（第2の5）、オ列長音（第2の6）、
  助詞「は・を・へ」（第2の1）、動詞「いう」（第2の2）——を実装しています。
