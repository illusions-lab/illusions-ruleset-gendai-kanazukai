...
3. `docs/rules/<ruleId>.md` を [docs/rules/gk-yotsugana.md](./rules/gk-yotsugana.md) の形式に
   従って作成する（**ルールごとに1ファイル**）。
...
- `docs/rules/` のファイル名は `ruleId` と一致させる（例 `gk-yotsugana` → `docs/rules/gk-yotsugana.md`）。
